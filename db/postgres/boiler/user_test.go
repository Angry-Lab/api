...
func testUserToManyAuthTokens(t *testing.T) {
	...
	if err = randomize.Struct(seed, &b, authTokenDBTypes, false, authTokenColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, authTokenDBTypes, false, authTokenColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.AuthTokens().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadAuthTokens(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	...
	a.R.AuthTokens = nil
	if err = a.L.LoadAuthTokens(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.AuthTokens); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpAuthTokens(t *testing.T) {
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddAuthTokens(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		count, err := a.AuthTokens().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
